      ...
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          ...
          <View style={styles.card}>
            <View style={styles.cardHeaderWithEdit}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('UserDetailScreen', { fromProfileEdit: true })}>
                <MaterialCommunityIcons name="pencil-outline" size={22} color="#222" />
              </TouchableOpacity>
            </View>
          ...
          <View style={styles.card}>
            ...
            {renderInfoRow('MaterialCommunityIcons', 'smoke', 'Số điếu tránh được', `${cigarettesAvoided} điếu`)}
            {renderInfoRow('MaterialCommunityIcons', 'cash-multiple', 'Tiền tiết kiệm', `${formatMoney(moneySaved)} VNĐ`)}
            {renderInfoRow('MaterialCommunityIcons', 'numeric', 'Điếu/ngày trước đây', user.smokingData?.cigaretteCount ? `${user.smokingData.cigaretteCount} điếu` : 'Chưa cập nhật')}
            {renderInfoRow('MaterialCommunityIcons', 'currency-usd', 'Giá 1 bao', user.smokingData?.pricePerPack ? `${formatMoney(user.smokingData.pricePerPack)} VNĐ` : 'Chưa cập nhật')}
            {renderInfoRow('MaterialCommunityIcons', 'package-variant', 'Gói/tuần', user.smokingData?.packsPerWeek ? `${user.smokingData.packsPerWeek} gói` : 'Chưa cập nhật')}
          ...
          <View style={styles.card}>
            ...
            {membershipStatus?.status !== 'active' && (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
              >
                <LinearGradient colors={["#43e97b", "#27ae60"]} style={styles.membershipButtonGradient}>
