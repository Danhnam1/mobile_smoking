...
} from "../api/user"; // Import fetchSmokingStatus API and getCurrentUserMembership API
        ...
        if (storedUser && storedToken) {
          ...
          if (userData._id && storedToken) {
            try {
              ...
              console.warn(
                "Could not fetch smoking status:",
                smokingError.message
              );
      ...
      try {
        ...
        if (userData._id && userToken) {
          try {
            ...
            console.warn(
              "Could not fetch smoking status after login:",
              smokingError.message
            );
          }
        }
    ...
    async (updatedData) => {
      try {
        ...
        if (updatedData.fullName)
          userProfileUpdates.full_name = updatedData.fullName;
        if (updatedData.dateOfBirth)
          userProfileUpdates.birth_date = updatedData.dateOfBirth;
        if (updatedData.gender) userProfileUpdates.gender = updatedData.gender;
        if (typeof updatedData.isProfileComplete !== "undefined")
          userProfileUpdates.isProfileComplete = updatedData.isProfileComplete;

        console.log(
          "User profile updates to send to backend:",
          userProfileUpdates
        );
        ...
        if (Object.keys(userProfileUpdates).length > 0) {
          ...
          console.log(
            "updateUser API call successful, received:",
            updatedUserFromApi
          );
        } else {
